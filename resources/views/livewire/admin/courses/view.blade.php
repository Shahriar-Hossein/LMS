<div class="max-w-6xl mx-auto">
    <div class="mb-6">
        <a href="{{ route('admin.courses.index') }}" class="text-sm text-emerald-600 dark:text-emerald-400 hover:underline mb-4 inline-block">
            ← Back to Courses
        </a>
        <div class="bg-white/80 dark:bg-zinc-900/80 rounded-2xl shadow-xl border border-emerald-100 dark:border-zinc-700 overflow-hidden">
            ...
            <div class="p-6">
                ...
                <div class="flex flex-wrap items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
                    ...
                    <span>Enrolled Students: <span class="font-medium text-emerald-600 dark:text-emerald-400">{{ $course->students_count ?? 0 }}</span></span>
                    <span>Price: <span class="font-medium text-emerald-600 dark:text-emerald-400">{{ $course->price ? '$' . $course->price : 'Free' }}</span></span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white/80 dark:bg-zinc-900/80 rounded-2xl shadow-xl border border-emerald-100 dark:border-zinc-700 p-6 mb-6">
        <h2 class="text-xl font-bold text-cyan-700 dark:text-emerald-300 mb-4">Course Status</h2>

        @if (session()->has('message'))
            <div class="mb-4 text-sm text-emerald-700 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded px-4 py-3">
                {{ session('message') }}
            </div>
        @endif
        ...
        <div class="flex items-end gap-4">
            <div class="flex-1">
                ...
                <select 
                    ...
                    class="w-full rounded-md border border-emerald-200 dark:border-zinc-700 bg-white/80 dark:bg-zinc-900/80 text-sm text-gray-900 dark:text-gray-100 px-3 py-2 focus:ring-emerald-500 focus:border-emerald-500">
        ...
        <div class="mt-3 text-xs text-gray-500 dark:text-gray-400">
            <p><strong>Draft:</strong> Course is not visible to students</p>
            <p><strong>Review:</strong> Course is under admin review</p>
            <p><strong>Published:</strong> Course is visible to students</p>
        </div>
    </div>
                    ...
                    <div class="border border-emerald-200 dark:border-zinc-700 rounded-lg overflow-hidden">
                        ...
                        @if(in_array($module->id, $expandedModules))
                            <div class="p-4 bg-white dark:bg-zinc-900 border-t border-emerald-100 dark:border-zinc-700">
                                ...
                                @if($module->lessons->isNotEmpty())
                                    ...
                                    <ul class="space-y-2 mb-4">
                                        @foreach($module->lessons as $lesson)
                                            <li>
                                                <div class="flex items-center gap-3 p-3 rounded-lg bg-cyan-50 dark:bg-cyan-900/20 border border-cyan-200 dark:border-cyan-800">
                                ...
                                <!-- Assignments (Display only, no submission section for admin) -->
